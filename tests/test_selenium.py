...
class PageFactory(object):
    ...
    def text_to_be_present_in_element(self,text,timeout=None):
        """
        Wait till the text to be present in element
        :param: text to be verified with element
        """
        if timeout is None:
            timeout = PageFactory().timeout
        return WebDriverWait(self.parent, timeout).until(
            EC.text_to_be_present_in_element(self._locator,text_=text)
        )

    def element_located_selection_state_to_be(self,state,timeout=None):
        """
        Wait till element to be located and check if the selection state specified is in that state
        :param: state to verified with an element in boolean
        """
        if timeout is None:
            timeout = PageFactory().timeout
        return WebDriverWait(self.parent, timeout).until(
            EC.element_located_selection_state_to_be(self._locator,is_selected=state)
        )
...
WebElement.text_to_be_present_in_element = PageFactory.text_to_be_present_in_element
WebElement.element_located_selection_state_to_be = PageFactory.element_located_selection_state_to_be
